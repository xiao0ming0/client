...
func initLogger() {
	...
	if runtime.GOOS == "windows" {
		...
		// Alpine Linux: 使用/var/log目录，如果/root不存在
		if _, err := os.Stat("/root"); os.IsNotExist(err) {
			logPath = "/var/log/server_watch.log"
		}
		// 确保日志目录存在
		if lastSlash := strings.LastIndex(logPath, "/"); lastSlash > 0 {
			if dir := logPath[:lastSlash]; dir != "" {
...
// 获取Alpine版本信息
func getAlpineVersion() string {
	// 优先读取/etc/alpine-release
	if data, err := os.ReadFile("/etc/alpine-release"); err == nil {
		version := strings.TrimSpace(string(data))
		return fmt.Sprintf("Alpine Linux %s", version)
	}
	
	// 读取/etc/os-release获取详细信息
	if file, err := os.Open("/etc/os-release"); err == nil {
		defer file.Close()
		scanner := bufio.NewScanner(file)
...
func getSystemVersion() string {
	...
	if isAlpineLinux() {
		...
		info, err := host.Info()
		if err != nil {
			return fmt.Sprintf("%s %s", alpineVersion, runtime.GOARCH)
		}
		// 组合Alpine版本和内核信息
		return fmt.Sprintf("%s (Kernel: %s)", alpineVersion, info.PlatformVersion)
	}
