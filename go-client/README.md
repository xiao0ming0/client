...
- **Alpine Linux**: 完全支持，自动检测Alpine版本信息
...
### Alpine Linux支持
- 自动检测Alpine Linux系统
- 从 `/etc/alpine-release` 或 `/etc/os-release` 读取版本信息
- 日志文件自动适配（如果 `/root` 不存在，使用 `/var/log/server_watch.log`）
- 完整的系统监控功能支持（CPU、内存、磁盘、网络等）
...
### 日志位置
- **Linux**: `/root/server_watch.log`（默认）或 `/var/log/server_watch.log`（如果/root不存在）
- **Alpine Linux**: `/root/server_watch.log`（如果/root存在）或 `/var/log/server_watch.log`（自动检测）
